...
class UnifiedSecurityScanner:
    ...
    def __init__(self, target_url, auth_config=None, wordlist_base=None):
        ...
        self.results = {
            ...
            'bypasses': [],
            'endpoints': [],
            'technologies': {},  # Fix: Inizializzazione sicura
    ...
    def crawl_with_bypasses(self, discovery_limit, skip_discovery):
        ...
        def crawl_page_with_bypass(url, depth=0):
            ...
            try:
                ...
                # IMPORTANTE: allow_redirects=True per seguire i redirect
                response = self.crawler.session.get(url, timeout=10, verify=False, allow_redirects=True)
                
                # Check if it was a redirect that ended in 403/401
                actual_status = response.status_code
                redirect_chain = []
                
                if response.history:
                    # Log redirect chain
                    redirect_chain = [(r.status_code, r.url) for r in response.history]
                    logger.info(f"Redirect chain: {redirect_chain} → {actual_status} {response.url}")
                
                # If 403/401, try bypasses
                if actual_status in [401, 403]:
                    logger.info(f"Protected resource detected: {url} ({actual_status})")
                ...
                # Save endpoint regardless of status
                endpoint_data = {
                    'url': response.url,  # Use final URL after redirects
                    'original_url': url if url != response.url else None,
                    'method': 'GET',
                ...
                # Add to results
                if endpoint_data not in self.results['endpoints']:
    ...
    def analyze_vulnerabilities(self):
        ...
        # Fix: Gestione sicura delle tecnologie
        technologies = {}
        if hasattr(self.crawler, 'results') and 'technologies' in self.crawler.results:
            technologies = self.crawler.results['technologies']
        elif 'technologies' in self.results:
            technologies = self.results['technologies']
        else:
            technologies = {}  # Safe fallback for interrupted scans
